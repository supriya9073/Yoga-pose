...
# --- Configuration Constants (MUST match your data generation) ---
IMAGE_SIZE = (224, 224) 
NUM_NUMERICAL_FEATURES = 47 

# --- Global Paths (MUST match 1_prepare_still_image_dataset.py) ---
...
class YogaPoseFrameDataset(Dataset):
    ...
    def _load_data(self):
        ...
        print(f"Found classes in {self.data_dir}: {self.idx_to_class}")
            ...
            for filename in sorted(os.listdir(class_path)):
                if filename.endswith(('.jpg', '.png', '.jpeg', '.bmp', '.tiff')):
                    ...
                    if os.path.exists(feature_path):
                        ...
                        print(f"Warning: Missing .npy feature file for {img_path}. Skipping this sample.")
        
        print(f"Loaded {len(self.image_paths)} individual images and features from {self.data_dir}")

    def _load_class_feature_means(self):
        if not os.path.exists(CLASS_FEATURE_MEANS_FILE):
            raise FileNotFoundError(f"Class feature means file not found: {CLASS_FEATURE_MEANS_FILE}. Please run 1_prepare_still_image_dataset.py first.")
        
        with open(CLASS_FEATURE_MEANS_FILE, 'r') as f:
            self.class_feature_means = json.load(f)
        print(f"Loaded class-wise feature means from {CLASS_FEATURE_MEANS_FILE}")
    ...
    def __getitem__(self, idx):
        ...
        if self.class_feature_means is not None:
            # Iterate through feature names to correctly impute
            # Get the list of feature names from the first class's means (assuming all classes have same features)
            feature_names_for_imputation = list(self.class_feature_means[label_string].keys())
            
            for i, feature_name in enumerate(feature_names_for_imputation):
                if torch.isnan(numerical_features_tensor[i]):
                    imputed_value = self.class_feature_means[label_string].get(feature_name, 0.0) 
                    numerical_features_tensor[i] = imputed_value
        else:
            numerical_features_tensor[numerical_features_tensor.isnan()] = 0.0
            print("Warning: Class feature means not loaded for imputation. Falling back to 0.0 for NaNs.")
...
def get_dataloaders(data_root, batch_size, image_size):
    """
    Creates and returns training and validation DataLoaders for multimodal still-image classification.
    Assumes data_root points to the STILL_IMAGE_DATASET_ROOT.
    """
    
    train_data_path = os.path.join(data_root, 'train')
    # --- FIX: Change 'val' to 'valid' to match folder created by 1_prepare_still_image_dataset.py ---
    val_data_path = os.path.join(data_root, 'valid') 
    # --- END FIX ---

    if not os.path.exists(train_data_path):
        raise FileNotFoundError(f"Training data path not found: {train_data_path}. Please ensure 1_prepare_still_image_dataset.py ran correctly.")
    if not os.path.exists(val_data_path):
        raise FileNotFoundError(f"Validation data path not found: {val_data_path}. Please ensure 1_prepare_still_image_dataset.py ran correctly.")

    print(f"Creating training dataset from: {train_data_path}")
    ...
    print(f"Creating validation dataset from: {val_data_path}")
    ...
    print(f"Train samples: {len(train_dataset_obj)}, Validation samples: {len(val_dataset_obj)}")
