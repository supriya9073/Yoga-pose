...
from models import QuadtreeCNN, get_model 
from dataloader import YogaPoseFrameDataset, IMAGE_SIZE # Import YogaPoseFrameDataset and IMAGE_SIZE

# --- Configuration ---
# IMPORTANT: Set this to the mode of the model you want to visualize!
# 'fusion', 'image_only', or 'numerical_only'
VISUALIZATION_MODE = 'image_only' 

# Path to your trained model weights
MODEL_PATH = f'quadtree_cnn_pose_model_{VISUALIZATION_MODE}.pth' 
...
# Output directory for saved Grad-CAM images
...
def extract_and_process_features(image_rgb, img_w, img_h):
    ...
    if results.pose_landmarks:
        landmarks = results.pose_pose_landmarks
...
def generate_grad_cam_heatmap(model, image_input_tensor, numerical_input_tensor, target_class=None):
    model.eval()
    
    image_input_tensor_grad = image_input_tensor.clone().requires_grad_(True) 

    hook_handle_activation = model.base_cnn.layer4.register_forward_hook(model.save_activation_hook)
    hook_handle_gradient = model.base_cnn.layer4.register_full_backward_hook(model.save_gradient_hook)
...
if __name__ == '__main__':
    ...
    train_dataset = YogaPoseFrameDataset(train_data_path, IMAGE_SIZE, is_train=False) # Use is_train=False to avoid augmentation
    ...
    for i, (image_tensor, numerical_features_tensor, label_tensor) in enumerate(tqdm(train_loader, desc="Generating Grad-CAMs")):
        ...
        with torch.no_grad(): 
            ...
            confidence_score_initial = confidence_initial.item()

        # Generate Grad-CAM Heatmap
        target_layer_for_cam = model.base_cnn.layer4 
        heatmap = generate_grad_cam_heatmap(model, image_tensor, numerical_features_tensor, target_class=predicted_idx_initial.item())
        ...
        # Use a unique name for the saved image, including original filename and predicted label
